...
Feature: Supplementary Manual QA - Media Handling
  ...
  Scenario: Number Chooser
    Given user is on the "Create New" page
      ...
      And user selects "Media Resource" from dropdown in "Identification Number" row
  ...
  Scenario: Record Title
    Given user is on the "Create New" page
      ...
      And user enters "testing" in the "Media Handling" "Title" field
      # following line does not exist
      And user clicks on the "Description" field
  ...
  Scenario: Folding/Unfolding Boxes
    ...
    Then the "Fold" symbol next to "File Information" should be an unfolded symbol
    ...
    Then the "Fold" symbol next to "Media Handling Information" should be an unfolded symbol
    Then close the browser
  ...
  Scenario: Vocabulary Pivoting
    ...
    Then "rightsHolder" should appear in the Terms Used sidebar
